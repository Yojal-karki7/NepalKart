...
const Login = () => {
  ...
  const handleSubmit = async(e)=>{
    ...
    try {
      if(currentState === 'Sign Up') {
        ...
        if(response.data.success) {
          setToken(response.data.token)
          localStorage.setItem('nepalkart_token', response.data.token)
        }
        else{
          toast.error(response.data.message)
        }
  ...
  return (
    <form onSubmit={handleSubmit} className='flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800'>
      ...
      <input type="password" onChange={(e)=>setPasssword(e.target.value)} value={password} className='w-full px-3 py-2 border border-gray-800 ' placeholder='Password' required />
